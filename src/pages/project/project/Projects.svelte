...
<div class="mt-8 mb-6 lg:mt-2 flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
    {#each data.filter(doc => {return doc.type==='project'}) as p}
        <div class="card p-2 cursor-pointer hover:shadow-none duration-300 hover:bg-white m-4" style="background-color: {p.color}" on:click={()=>project=p}>
            <h2 class="group flex">
                {p.name}
                <Icon icon="edit" classes="h-6 w-6 group-hover:visible invisible" on:clicked={()=>dispatch('data', p)}/>
            </h2>
            <ul class="m-1">
