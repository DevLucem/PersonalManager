...
<div class="card mt-4 pb-2 pt-4 px-4 flex-1">
    ...
    <div class="flex flex-col justify-end items-center flex-1">
        <div class="flex justify-between w-full">
            <button on:click={()=>completed=!completed} class="{completed?'text-gray':'text-gray'} text-center text-sm px-8 py-1 rounded-lg font-bold border rounded-full border-gray">0 Paused</button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2">
                {#each projects.filter(p => p.done) as project}
                    <div class="border border-gray rounded-xl p-4 m-2 cursor-pointer">
                        ...
                        <button on:click={()=>{if (confirm("UnMark This Project As Complete?")) PROJECTS.doc(project.id).update({done: null})}} class="text-sm font-bold uppercase mt-4">Grind</button>
                    </div>
                {/each}
            </div>
        {/if}
